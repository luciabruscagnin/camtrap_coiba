...
agoutigross <- read.csv("agouti_output/coiba-national-park-tool-use-20220404142022/observations.csv", header = TRUE)

# open the associated deployment keys (also downloaded from agouti.eu)
depl_keys <- read.csv("agouti_output/coiba-national-park-tool-use-20220404142022/deployments.csv", header = TRUE)
...
multimedia <- read.csv("agouti_output/coiba-national-park-tool-use-20220404142022/media.csv", header = TRUE)
...
# can also filter out the cameraSetup sequences (using that variable)
...
hist(hour(agoutisequence$seq_start[agoutisequence$capuchin == 1]), xlab = "Time of Day", ylab = "Number of Sequences with Capuchins", main = "Capuchin Detections Across All Locations")
## Per location
